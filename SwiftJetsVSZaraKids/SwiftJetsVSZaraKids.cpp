...
#define COLOR_LILAC "\033[38;5;183m" //Player: Taylor Swift
#define COLOR_PASTEL_BLUE "\033[38;5;123m" //Player: Amancio Ortega
#define COLOR_YELLOW "\033[38;5;226m" //Card: Name
#define COLOR_RED "\033[38;5;196m" //Attack Failed, Healing Failed, Dice Roll Failed or Invalid Choice
#define COLOR_GREEN "\033[38;5;46m" //Atack Successful, Healing Auccessful or Dice Roll Successful
#define COLOR_ORANGE "\033[38;5;208m" //Effect on moralitos (by Event or Action cards)
#define COLOR_PINK "\033[38;5;205m" //Arbolito gained
...
public:
    ...
    virtual void display() const {
        cout << COLOR_YELLOW << "Card: " << name << COLOR_RESET
            << ", Type: " << type << endl;
    }
    ...
    void applyEffect(int& currentMoralitos, int& opponentMoralitos, int& arbolitos, int& diceRoll, bool& wildcardUsed) const override {
        ...
        do {
            ...
            if (choice == 'A' || choice == 'a') {
                do {
                    if (arbolitos > 0) {
                        ...
                        if (useArbolito == 'Y' || useArbolito == 'y') {
                            ...
                            cout << COLOR_GREEN << "Attack successful using an arbolito! Remaining arbolitos: " << arbolitos << COLOR_RESET << endl;
                        }
                        else if (useArbolito == 'N' || useArbolito == 'n') {
                            if (diceRoll >= attackThreshold) {
                                opponentMoralitos -= attackThreshold;
                                cout << COLOR_GREEN << "Attack successful! Dice roll: " << diceRoll << COLOR_RESET << endl
                                    << COLOR_ORANGE << " -" << attackThreshold << " moralitos to opponent." << COLOR_RESET << endl;
                            }
                            else {
                                cout << COLOR_RED << "Attack failed! Dice roll: " << diceRoll << COLOR_RESET << endl;
                            }
                        }
                        else {
                            cout << COLOR_RED << "Invalid choice, please enter Y or N." << COLOR_RESET << endl;
                        }
                    }
                    else {
                        if (diceRoll >= attackThreshold) {
                ...
                } while (useArbolito != 'Y' && useArbolito != 'y' && useArbolito != 'N' && useArbolito != 'n');
            }
                ...
                do {
                    if (arbolitos > 0) {
                        ...
                        if (useArbolito == 'Y' || useArbolito == 'y') {
                            ...
                            cout << COLOR_GREEN << "Healing successful using an arbolito! Remaining arbolitos: " << arbolitos << COLOR_RESET << endl;
                        }
                        else if (useArbolito == 'N' || useArbolito == 'n') {
                            if (diceRoll >= healThreshold) {
                                currentMoralitos += healThreshold;
                                cout << COLOR_GREEN << "Healing successful! Dice roll: " << diceRoll << COLOR_RESET << endl
                                    << COLOR_ORANGE << " +" << healThreshold << " moralitos to self." << COLOR_RESET << endl;
                            }
                            else {
                                cout << COLOR_RED << "Healing failed! Dice roll: " << diceRoll << COLOR_RESET << endl;
                            }
                        }
                        else {
                            cout << COLOR_RED << "Invalid choice, please enter Y or N." << COLOR_RESET << endl;
                ...
                cout << COLOR_RED << "Invalid choice, try again." << COLOR_RESET << endl << endl;
            }
...
public:
    ...
    void display() const override {
        Card::display();
        cout << COLOR_ORANGE << "Effect on moralitos: ";
        if (effect >= 0) cout << "+";
        cout << effect << COLOR_RESET << endl << endl;
    }
...
public:
    ...
    void display() const override {
        Card::display();
        cout << "Effect: +1 arbolito" << endl << endl;
    }

    void applyEffect(int& currentMoralitos, int& opponentMoralitos, int& arbolitos, int& diceRoll, bool& wildcardUsed) const override {
        arbolitos++;
        cout << COLOR_PINK << "Arbolito gained! Total arbolitos: " << arbolitos << COLOR_RESET << endl;
    }
};
...
int main() {
    ...
    // Create players
    Player player1(COLOR_LILAC"Taylor Swift");
    Player player2(COLOR_PASTEL_BLUE"Amancio Ortega");
    ...
    while (!deck.isEmpty() && player1.moralitos > 0 && player2.moralitos > 0) {
        ...
        char ready;
        cout << COLOR_RESET << "Are you ready for the next turn? Write anything and press enter to continue" << endl;
        cin >> ready;
    }
